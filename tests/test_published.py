...
CACHE_DIR = pathlib.Path(CUR_DIR, "container_cache")
...
def get_exp(name, sfx):
    exp_path = pathlib.Path(EXP_DIR, "%s%s.out" % (name, sfx))
...
def container(tmp_path_factory):
    ...
    def get_container(name):
        ...
        path = pathlib.Path(CACHE_DIR, "%s.bin" % name)
        try_download = False
        ...
        if try_download:
            ...
            tmp = pathlib.Path(dl, "%s.%s" %
                               (name, "out" if tar_path is None else "tar"))
            ...
            if tar_path is not None:
                with tarfile.open(tmp, "r:*") as tar:
                    tmp = pathlib.Path(dl, "%s.bin" % name)
        ...
        rel_path = os.path.relpath(path, AUI_DIR)
...
@pytest.fixture(scope="module", params=CONTAINER_SETS, ids=lambda x: x[0])
def concat_container(request, container, tmp_path_factory):
    name, containers, exp_csum = request.param
    ...
    with open(tmp, mode="wb") as t:
        for cname in containers:
            ...
            with open(pathlib.Path(AUI_DIR, cpath), "rb") as c:
                t.write(c.read())
...
def __test_container(capfd, container, args):
    name, path = container
    exp = get_exp(name, "".join(args))
    subprocess.run([AUI_PATH, ] + args + [path, ], cwd=AUI_DIR)
...
@pytest.mark.parametrize("args", [([]), (["-v"]), (["-v", "--verbose"])],
                         ids=lambda x: "".join(x))
def test_released_container(capfd, released_container, args):
    __test_container(capfd, released_container, args)

@pytest.mark.parametrize("args", [([]), (["--verbose"]), (["-vv"])],
                         ids=lambda x: "".join(x))
def test_concatenated_container(capfd, concat_container, args):
    __test_container(capfd, concat_container, args)
